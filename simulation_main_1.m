...
Mid=(P+1)/2;
% Parameter values
D=0.0002; h1=20;HH1=h1/dx;h2=9;HH2=h2/dx;
tmax=20000;
dt=0.5;
...
S1=N1-0*(1-x+x); I1=0*(1-sin(x));R1=0*(1-x+x);I1(Mid-1:Mid+1)=0;
S11=S1; I11=I1;R11=R1;
S2=N2-0*rand*(1-x+x); I2=0*(1-sin(x));R2=0*(1-x+x);
for j=Mid-1:Mid+1
I2(j)=20;
end

S22=S2; I22=I2;R22=R2;
...
%      J2(j-HH2)=(dx/2)*(U2(j-HH2)+U2(j+HH2)+2*sum(U2(j-HH2+1:j+HH2-1)))/(2*h2);
 J2(j-HH2)=(dx/2)*(U2(j-HH2)+U2(j+HH2)+2*sum(U2(j-HH2+1:j+HH2-1)));
    ...
    S10=N1-(dx/2)*(I1(1)+R1(1) +I1(P)+R1(P) + 2*sum(I1(2:P-1)+R1(2:P-1)));

    S20=N2-(dx/2)*(I2(1)+R2(1) +I2(P)+R2(P) + 2*sum(I2(2:P-1)+R2(2:P-1)));
 S101=(dx/2)*(I1(1)+R1(1) +I1(P)+R1(P) + 2*sum(I1(2:P-1)+R1(2:P-1)));
 S201=(dx/2)*(I2(1)+R2(1) +I2(P)+R2(P) + 2*sum(I2(2:P-1)+R2(2:P-1)));
    for i=1:P
    S1(i)=S10+S101-J1(i);
