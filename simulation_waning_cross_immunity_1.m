...
Mid=(P+1)/2;
% Parameter values
D=0.002; h1=15;HH1=h1/dx;h2=9;HH2=h2/dx;
tmax=20000;
dt=0.05;
...
MM=tmax/dt;
N1=10^8;N2=5*10^8;
beta11=0.9/N1; beta12=0.000001/N1; beta2=0.82/N2; gamma1=0.85; nu1=0.0; gamma2=0.78; nu2=0.0;a1=0.004;a2=0.003;
...
S1=N1-0*(1-x+x); I1=0*(1-sin(x));R1=0*(1-x+x);D1=0*(1-x+x);I1(Mid-1:Mid+1)=0;m1=0*(1-x+x);
S11=S1; I11=I1;R11=R1;D11=D1;m11=m1;
S2=N2-0*rand*(1-x+x); I2=0*(1-sin(x));R2=0*(1-x+x);D2=0*(1-x+x);m2=0*(1-x+x);
for j=Mid-1:Mid+1
I2(j)=20;
end
% for j=1:5
...
% I2(j)=20;
% end
% for j=Mid+100:Mid+101
% I2(j)=20;
% end
  
S22=S2; I22=I2;R22=R2;D22=D2;m22=m2;
W11=[];W12=[];W13=[];W14=[];W21=[];W22=[];W23=[];W24=[];W15=[];W25=[];
...
%   U1(1:HH1)=I1(1)+R1(1); U1(HH1+1:P+HH1)=I1(1:P)+R1(1:P);U1(P+HH1+1:P+2*HH1)=I1(P)+R1(P);
    U1(1:HH1)=0*(I1(1)+m1(1)); U1(HH1+1:P+HH1)=I1(1:P)+m1(1:P);U1(P+HH1+1:P+2*HH1)=0*(I1(P)+m1(P));
%    J(1)=(dx/2)*(U(HH+1-HH)+U(HH+1+HH)+2*sum(U(HH+1-HH+1:HH+1+HH-1)))/(2*h);
...
% 
%    U2(1:HH2)=I2(1)+R2(1); U2(HH2+1:P+HH2)=I2(1:P)+R2(1:P);U2(P+HH2+1:P+2*HH2)=I2(P)+R2(P);

   U2(1:HH2)=0*(I2(1)+m2(1)); U2(HH2+1:P+HH2)=I2(1:P)+m2(1:P);U2(P+HH2+1:P+2*HH2)=0*(I2(P)+m2(P));
...
%      J2(j-HH2)=(dx/2)*(U2(j-HH2)+U2(j+HH2)+2*sum(U2(j-HH2+1:j+HH2-1)))/(2*h2);
 J2(j-HH2)=(dx/2)*(U2(j-HH2)+U2(j+HH2)+2*sum(U2(j-HH2+1:j+HH2-1)));
    ...
    S10=N1-(dx/2)*(D1(1) +D1(P) + 2*sum(D1(2:P-1)));

    S20=N2-(dx/2)*(D2(1) +D2(P) + 2*sum(D2(2:P-1)));
 S101=(dx/2)*(I1(1)+m1(1) +I1(P)+m1(P) + 2*sum(I1(2:P-1)+m1(2:P-1)));
 S201=(dx/2)*(I2(1)+m2(1) +I2(P)+m2(P) + 2*sum(I2(2:P-1)+m2(2:P-1)));
    for i=1:P
    S1(i)=S10-J1(i);
...
m22(1)=m2(1)+dt*(gamma2*I2(1)-a2*m2(1));
 ...
 m22(i)=m2(i)+dt*(gamma2*I2(i)-a2*m2(i));
    end
...
%        ZZ(P)=Z(P)+2*r*(Z(P-1)-Z(P))+dt*(beta*Z(P)*Y(P)-(mu+nu)*Z(P));
       ...
       D22(P)=D2(P)+dt*(nu2*I2(P));
        m22(P)=m2(P)+dt*(gamma2*I2(P)-a2*m2(P));
     ...
     m11(i)=m1(i)+dt*(gamma1*I1(i)-a1*m1(i));
    end
%       Y=YY;
%       Z=ZZ;
%       Q=QQ; 
