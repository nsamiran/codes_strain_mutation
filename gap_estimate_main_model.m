...
%      J2(j-HH2)=(dx/2)*(U2(j-HH2)+U2(j+HH2)+2*sum(U2(j-HH2+1:j+HH2-1)))/(2*h2);
 J2(j-HH2)=(dx/2)*(U2(j-HH2)+U2(j+HH2)+2*sum(U2(j-HH2+1:j+HH2-1)));
    ...
    S10=N1-(dx/2)*(I1(1)+R1(1)+D1(1) +I1(P)+R1(P)+D1(P) + 2*sum(I1(2:P-1)+R1(2:P-1)+D1(2:P-1)));

    S20=N2-(dx/2)*(I2(1)+R2(1)+D2(1) +I2(P)+R2(P)+D2(P) + 2*sum(I2(2:P-1)+R2(2:P-1)+D2(2:P-1)));
...
%     end
    ...
    if mod(n,10)==0
       CC=CC+1
        n=n
         subplot(2,1,1)
        plot(x,R1,'b-','LineWidth',2);
         subplot(2,1,2)
%         plot(x,S1,'b-','LineWidth',2); 
%         subplot(4,1,3)
         plot(x,R2,'r-','LineWidth',2);
%         subplot(4,1,4)
...
%         WWW2(n)=sum(I2);
   ...
   NEW_CONSTANT=NEW_CONSTANT+ (beta2*S2.*I2)-I2;
end
